...
Subirlo al Arduino.
Cerrar el Monitor Serial antes de ejecutar el servidor Python.
...
servidor_central.py se encuentre ejecutándose.
La terminal del servidor muestre lecturas.
Se haya indicado correctamente el número de habitaciones.
El Arduino esté conectado al puerto correcto.
La base de datos esté siendo actualizada.
La página se haya abierto en http://127.0.0.1:5001.
Falta ReportLab
...
Ejecutar servidor_central.py.
Indicar la cantidad de habitaciones.
Confirmar que la terminal muestra mediciones.
Ejecutar web/app.py.
Abrir la página web.
Ejecutar grafica_tiempo_real.py.
Compilar y ejecutar el cliente Java.
Ejecutar el asistente HAL si se necesita.
